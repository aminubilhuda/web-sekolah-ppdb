...
@section('meta_description', 'Prestasi-prestasi yang telah diraih oleh siswa dan sekolah kami')

@section('content')
<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold text-gray-900 mb-8">Prestasi</h1>

    <!-- Filter Kategori -->
    <div class="mb-8">
        <div class="flex space-x-4">
            <a href="{{ route('web.prestasi.index') }}" 
               class="px-4 py-2 rounded-lg {{ !request('kategori') ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
                Semua
            </a>
            <a href="{{ route('web.prestasi.akademik') }}" 
               class="px-4 py-2 rounded-lg {{ request()->routeIs('web.prestasi.akademik') ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
                Akademik
            </a>
            <a href="{{ route('web.prestasi.non-akademik') }}" 
               class="px-4 py-2 rounded-lg {{ request()->routeIs('web.prestasi.non-akademik') ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
                Non-Akademik
            </a>
        </div>
    </div>
